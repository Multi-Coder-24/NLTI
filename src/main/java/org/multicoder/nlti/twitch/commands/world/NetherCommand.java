...
public class NetherCommand {
    ...
    public static void PostTrigger(String Username,MinecraftServer server) {
        server.getPlayerManager().broadcast((Text.translatable("text.nlti.command",Username,"Nether")),false);
        COOLDOWN = LocalDateTime.now().plusSeconds(DELAY);
    }

    public static void Trigger(String Username,MinecraftServer server) {
        server.getPlayerManager().getPlayerList().forEach(player -> player.teleport(server.getWorld(ServerWorld.NETHER),0,72,0,new HashSet<>(),player.lastPitch,player.lastYaw,true));
